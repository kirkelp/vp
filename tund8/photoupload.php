    ...
    if(isset($_POST["photosubmit"])){
	  ...
	  $check = getimagesize($_FILES["photoinput"]["tmp_name"]);
		...
		if($_FILES["photoinput"]["size"] > 1048576){
			$error .= "Fail ületab lubatud suurused";
		}
		//loome failinime
		...
		if(file_exists($origphotodir .$filename)){
			$error .= "Selle nimega pilt on juba olemas";
		}
	   
	 if(empty($error)){
		 //teeme pildi väiksemaks
		 //teeme pildiobjekti-pikslikogum
		...
		//uurime pildi originaal suurust
		...
		//loome uue tühja pildiobjekti kuhu pikslid ümbertõsta
		...
		//salvestame pildifail
		if($filetype == "jpg"){
			if(imagejpeg($mynewimage, $normalphotodir .$filename, 90)){
				$notice = "vähendatud pildi salvestame õnnestus";				
			}else{
...
?>
  ...
  <h2>Laeme foto</h2>
		...
		<label for="privinput1">1 on privaatne (näen ise)</label>
		<input id="privinput2" name="privinput" type="radio" value="2">
		<label for="privinput2">2 on sisseloginud kasutajatele</label>
		<input id="privinput3" name="privinput" type="radio" value="3">
		<label for="privinput3">3 on avalik </label>
		<br>
